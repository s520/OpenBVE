...
namespace OpenBve
{
	static partial class Bve5ScenarioParser
	{
					...
					case "gauge":
						{
							...
							Game.RouteRailGauge = Convert.ToDouble(Gauge);
						}
						break;
		...
		private static void ConvertStation(MapData ParseData, RouteData RouteData)
		{
			...
			Game.Stations = new Game.Station[CurrentStation];
			...
			foreach (var Statement in ParseData.Statements)
			{
				...
				Sounds.SoundBuffer ArrivalSound;
				Sounds.SoundBuffer DepartureSound;
				...
				Game.Station NewStation = new Game.Station();
				...
				NewStation.Stops = new Game.StationStop[1];
				...
				NewStation.MaxInterferingObjectRate = Program.RandomNumberGenerator.Next(1, 99);
				...
				Game.Stations[CurrentStation] = NewStation;
				CurrentStation++;
			}
		}
		...
		private static void ConvertBackground(bool PreviewOnly, MapData ParseData, RouteData RouteData)
		{
			...
			foreach (var Statement in ParseData.Statements)
			{
				...
				if (BackgroundIndex == -1)
				{
					...
					if (Object != null)
					{
						RouteData.Backgrounds.Add(new Background(Convert.ToString(BackgroundKey), new BackgroundManager.BackgroundObject((ObjectManager.StaticObject)Object)));
					}
					else
					{
						RouteData.Backgrounds.Add(new Background(Convert.ToString(BackgroundKey), new BackgroundManager.StaticBackground(null, 6, false)));
					}
				}
					...
					case "fog":
						{
							...
							{
								Blocks[BlockIndex].Fog.Start = Game.NoFogStart;
								Blocks[BlockIndex].Fog.End = Game.NoFogEnd;
							}
