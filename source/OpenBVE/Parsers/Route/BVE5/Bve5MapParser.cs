...
namespace OpenBve
{
	static partial class Bve5ScenarioParser
	{
				...
				foreach (var error in Parser.ParserErrors.OrderBy(e => e.Line).ThenBy(e => e.Column))
				{
					switch (error.ErrorLevel)
					{
						case ParseErrorLevel.Error:
							Interface.AddMessage(MessageType.Error, false, string.Format("[{0}:{1}] {2}: {3} in {4}", error.Line, error.Column, error.ErrorLevel, error.Message, FileName));
							break;
						case ParseErrorLevel.Warning:
							Interface.AddMessage(MessageType.Warning, false, string.Format("[{0}:{1}] {2}: {3} in {4}", error.Line, error.Column, error.ErrorLevel, error.Message, FileName));
							break;
					}
				}
			}
		...
		private static void ParseIncludeMap(string FileName, string InputText, ref MapData Data)
		{
			...
			while (!IsAllInclude)
			{
				for (var i = IncludePosition; i < ModData.Statements.Count; i++)
				{
					...
					{
						Interface.AddMessage(MessageType.Error, false, IncludeFileName + "is not found.");
					}
		...
		private static void ConvertToBlock(string FileName, bool PreviewOnly, MapData ParseData, out RouteData RouteData)
		{
			...
			if (Interface.CurrentOptions.EnableBve5TFO)
			{
