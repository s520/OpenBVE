...
namespace OpenBve
{
	static partial class Bve5ScenarioParser
	{
		...
		{
			Interface.CurrentOptions.CurrentXParser = Interface.XParsers.Assimp;
			...
			Game.RouteComment = Data.Comment;
			if (!string.IsNullOrEmpty(Data.Image))
			{
				Game.RouteImage = Path.CombineFile(System.IO.Path.GetDirectoryName(FileName), Data.Image);
			}
		...
		private static int GetRandomIndex(params double[] WeightTable)
		{
			...
			double Value = Program.RandomNumberGenerator.NextDouble() * TotalWeight;
			int RetIndex = -1;
