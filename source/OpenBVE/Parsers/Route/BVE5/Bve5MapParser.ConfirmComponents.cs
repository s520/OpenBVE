	...
	static partial class Bve5ScenarioParser
	{
		...
		private static void ConfirmRepeater(bool PreviewOnly, MapData ParseData, RouteData RouteData)
		{
			...
			foreach (var Repeater in RepeaterList)
			{
				...
				if (Repeater.StartRefreshed)
				{
					double EndTrackPosition = Game.Stations.Last().Stops.First().TrackPosition + Interface.CurrentOptions.ViewingDistance;
					...
					case "beginnew":
						{
							...
							int StationIndex = Array.FindLastIndex(Game.Stations, s => s.Stops.Last().TrackPosition <= Statement.Distance);
							if (StationIndex != -1)
							{
								Station Station = RouteData.StationList.Find(s => s.Name.Equals(Game.Stations[StationIndex].Name, StringComparison.InvariantCultureIgnoreCase));
								if (Station != null)
								{
		...
		private static void ConfirmPreTrain(bool PreviewOnly, MapData ParseData)
		{
			...
			foreach (var Statement in ParseData.Statements)
			{
				...
				Array.Resize(ref Game.BogusPretrainInstructions, n + 1);

				Game.BogusPretrainInstructions[n].TrackPosition = Statement.Distance;
					...
					case "ambient":
						{
							...
							Renderer.OptionAmbientColor = new Color24((byte)(Red * 255), (byte)(Green * 255), (byte)(Blue * 255));
						}
						break;
					case "diffuse":
						{
							...
							Renderer.OptionDiffuseColor = new Color24((byte)(Red * 255), (byte)(Green * 255), (byte)(Blue * 255));
						}
						break;
					case "direction":
						{
